...
X = df_occurrence[['Occurrence_No', 'Occurrence_Description', 
...
train_data = X.merge(y_lv, how='inner', on='Occurrence_No').drop(
    ['Occurrence_No', 'Occurrence_Type_Code'], axis=1)
...
categories = {k:dict_ID_Text[k] for k in train_data.columns if k != 'Occurrence_Description'}
...
folds = []
c_fold = 0
X = np.asmatrix(train_data[['Occurrence_Description']].values)
y = np.asmatrix(train_data[list(categories.keys())].values)
...
for i in range(len(order)): 
    x_txt = order.value.values[i] + 0.1
    y_txt = i + 0.1
